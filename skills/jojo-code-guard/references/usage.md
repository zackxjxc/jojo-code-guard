...
Skill 自动加载后，日常修改无需输入命令。若项目根目录存在 `AGENTS.md`，Skill 会遵守其中规则；同时遵守 `.editorconfig`、`.gitattributes` 和（如果存在）`.vscode/settings.json`，保护旧文件原始编码、BOM 和换行，禁止无关格式化和大面积 diff。`AGENTS.md` 是可选文件，不会自动创建；用户需要时可自行创建并写入项目规则。各阶段的触发边界见 [生效与验收](生效与验收.md)。
...
Claude 插件在 Edit/Write 类工具完成后会由 PostToolUse 自动运行 `post-write-check`，将诊断反馈给 AI；
发现 BLOCKED 时会请求支持该协议的客户端暂停或替换当前工具结果，但由于它发生在写入后，不能撤销已经完成的写入。通过 Bash、外部脚本或其他客户端写文件时，AI 必须在每次写入后运行 `check_diff.py`；
Codex 使用原生 Skill Discovery；当前实测的 Codex 0.142.3 会发现插件的 `hooks/hooks.json`，执行仍取决于
Hook 功能和信任状态。Hook 从业务仓库的当前工作目录启动；Codex 注入 `PLUGIN_ROOT` 和兼容变量
...
Codex 插件安装或升级后重新打开会话，使主 Skill Discovery 生效；需要自动生命周期检查时，还要确认客户端已发现并信任 `hooks/hooks.json` 中的 Hook。Hook 内容变化的升级可能需要重新信任，日常会话不需要重复操作。
