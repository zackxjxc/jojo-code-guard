...
文件变化时立即停止后续交付和提交；可依据编辑前状态或 Git 基线安全恢复的，做最小修复并复检；
无法在不覆盖用户修改的前提下安全修复时，再报告并等待用户处理。若 Git 的
`core.autocrlf`/`core.eol` 可能改写工作区，先报告并要求修正；
...
真实生命周期执行只能在客户端内人工验收。
Skill 不会自行更新；doctor 发现新版本时提示用户通过对应客户端刷新 marketplace、重新安装插件并重启。
