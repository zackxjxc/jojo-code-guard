...
这是每个会话加载的轻量路由器。先执行暗号检测，再根据本轮实际任务加载所需规则；纯对话不绑定当前目录，
不读取项目配置，也不执行 Git、编码或差异检查。
...
每轮按用户意图和即将执行的操作判断，可组合加载多个模块。项目主要语言不能替代任务判断。

| 当前任务 | 必须完整读取 |
|---|---|
| 纯聊天、翻译、知识问答，且不访问本地文件或仓库 | 无；暗号检测后直接处理请求 |
| 只读查看本地项目 | 项目根目录 `AGENTS.md`（如果存在）；不执行编辑前后检查 |
| 创建、修改、移动、删除、格式化或生成任意本地文件 | [通用文件守护](references/通用文件守护.md) |
| 处理 C/C++、Visual Studio/MSVC 工程或资源文件 | 在通用守护之外读取 [C++ 专项规则](references/C++专项规则.md) |
| 编写、修改、评审或诊断 `.ps1`、`.psm1`、`.psd1`、`.bat`、`.cmd`，或设计复杂 PowerShell 进程/提权/重定向命令 | [PowerShell规则.md](PowerShell规则.md)；发生文件修改时同时读取通用守护 |
| 暂存、提交、推送、改写历史、配置 Git、安装 Hook 或审查提交边界 | [Git 操作规则](references/Git操作规则.md)；发生文件修改时同时读取通用守护 |
...
- 修改文件前必须先完整读取通用守护及所有命中的专项规则。所需资源缺失或无法完整读取时，暂停相关
  文件或仓库操作并报告；纯对话不因未命中的专项资源而暂停。
- 本次明确用户要求优先于项目规则；项目规则优先于模块默认值。文本规则与实际 Git 属性冲突时先报告，
  不静默覆盖。
