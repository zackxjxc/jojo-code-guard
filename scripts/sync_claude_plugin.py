...
CLAUDE_MANIFEST_ALLOWED_KEYS = frozenset(
    {
        "name",
        "description",
        "version",
...
CLAUDE_MARKETPLACE_ALLOWED_KEYS = frozenset({"name", "owner", "metadata", "plugins"})
CLAUDE_MARKETPLACE_PLUGIN_ALLOWED_KEYS = frozenset(
    {"name", "source", "description", "version", "strict"}
)
# 仅精确匹配本地发布历史生成的完整无 marker 适配包，不能按单个旧文件名取得删除授权。
LEGACY_PACKAGE_TREE_SHA256 = {
    "0.2.0": frozenset({"a953cd96bd377fd7b65f971b085583439a4b860aa067594b0c75956ed378fe45"}),
    "0.2.1": frozenset({"b6291895fd60937fb5472771044bd976d229b94ef9711cbf72a39e7e94b6846e"}),
    "0.2.2": frozenset({"86ba25a1f499f73499d579bf8c4e15dcf9edf8a4899182f2cd7b2446668d860d"}),
    "0.2.3": frozenset({"8561631c64ac7a2ec9e04e5b542382026bcb856a20a9f35d4b1b2292a9bc841a"}),
    "0.2.4": frozenset({"7d2c3d903dd076bafd251a3eee35e81b8f4ecfbcaa0a2760948c73547421753a"}),
    "0.2.5": frozenset({"943d20845801823ce83b99f503e026678f823ffd3cf3a699933ba140e47c8dbe"}),
    "0.2.6": frozenset({"664ae0eafb26cb96050d3a4bf58dafb9b9c5e0952ecc9081e280668743b9a510"}),
    "0.2.7": frozenset({"f42aa9f38a894b4a9ec434e7abacaee16a9b4a48c8a0ec11da50e3c43836aa94"}),
    "0.2.8": frozenset({"d6cef8288689d377e9910dacbfe44135535f1a3f5a997f6778d2ff92171c457b"}),
    "0.2.9": frozenset({"6617f32129286f60d9c607dbeaa154de486ff46b3116b6789becb97cd26615a0"}),
    "0.2.10": frozenset({"ac84158eec9b51b1081b7de616f215943fdfa5c0c981eb4e974983f362833853"}),
    "0.2.11": frozenset({"729cc370912b813ce75b075f1651c2c2aad0c60d42cfe0d0d9df59648fb18b1b"}),
    "0.2.12": frozenset({"95b84f807e6f465a972d5f24415b1a074d8d16712b277fda79bebeee46e10b4a"}),
    "0.2.13": frozenset(
        {
            "190705d7954310749c2c41106e5f0640f7d40b730b6b3d77a2f25575cc695b2c",
            "9b9d1ad3eee015a48d9c8391fe635e3b2bba8740c822d39a58060181e55f9ce8",
            "b17e70e35efd3d0439205cdd67a2dd20e8e6dca676c21ca002c62a0fcfedfb66",
            "fe3a5298229fe89bfe7fc9dcf8a0eaaceadcef7469247ae019883a49284a16b4",
            "68795dcbead0e188001267c9ea83e1183f7b186abec855a701cffbd9af895df3",
        }
    ),
}
...
def _validate_manifest(path: pathlib.Path) -> dict[str, object]:
    ...
    try:
        ...
        raise RuntimeError(f"Claude manifest 无法安全读取：{path}") from error
    if not isinstance(manifest, dict):
        raise RuntimeError(f"Claude manifest 顶层必须是对象：{path}")
    unexpected = sorted(set(manifest) - CLAUDE_MANIFEST_ALLOWED_KEYS)
    if unexpected:
        raise RuntimeError("Claude manifest 包含未允许字段：" + ", ".join(unexpected))
    if manifest.get("name") != PLUGIN_NAME:
        raise RuntimeError("Claude manifest 的插件名未允许")
    if not isinstance(manifest.get("version"), str) or not manifest["version"]:
        raise RuntimeError("Claude manifest 的版本无效")
    return manifest


def _validate_marketplace(path: pathlib.Path, plugin_manifest: dict[str, object]) -> None:
    """确认 marketplace 只能严格引用当前目录中的唯一插件。"""
    try:
        marketplace = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as error:
        raise RuntimeError(f"Claude marketplace 无法安全读取：{path}") from error
    if not isinstance(marketplace, dict):
        raise RuntimeError(f"Claude marketplace 顶层必须是对象：{path}")
    unexpected = sorted(set(marketplace) - CLAUDE_MARKETPLACE_ALLOWED_KEYS)
    if unexpected:
        raise RuntimeError("Claude marketplace 包含未允许字段：" + ", ".join(unexpected))
    if marketplace.get("name") != PLUGIN_NAME:
        raise RuntimeError("Claude marketplace 的名称未允许")
    ...
    if (
        ...
        raise RuntimeError("Claude marketplace owner 必须是含非空 name 的对象")
    plugins = marketplace.get("plugins")
    if not isinstance(plugins, list) or len(plugins) != 1 or not isinstance(plugins[0], dict):
        raise RuntimeError("Claude marketplace 必须且只能声明一个插件")
    plugin = plugins[0]
    unexpected_plugin = sorted(set(plugin) - CLAUDE_MARKETPLACE_PLUGIN_ALLOWED_KEYS)
    if unexpected_plugin:
        raise RuntimeError(
            "Claude marketplace 插件包含未允许字段：" + ", ".join(unexpected_plugin)
        )
    if plugin.get("name") != PLUGIN_NAME:
        raise RuntimeError("Claude marketplace 的插件名未允许")
    if plugin.get("source") != "./":
        raise RuntimeError("Claude marketplace 的插件 source 必须是 ./")
    if plugin.get("strict") is not True:
        raise RuntimeError("Claude marketplace 的插件 strict 必须为 true")
    if plugin.get("version") != plugin_manifest.get("version"):
        raise RuntimeError("Claude marketplace 与 plugin manifest 的版本不一致")
    metadata = marketplace.get("metadata")
    if isinstance(metadata, dict) and "version" in metadata:
        if metadata["version"] != plugin_manifest.get("version"):
            raise RuntimeError("Claude marketplace metadata 与 plugin manifest 的版本不一致")


def _component_entrypoints(root: pathlib.Path, suffix: str | None) -> list[pathlib.Path]:
    """枚举组件文件并阻止遍历目录链接。"""
    ...
    for directory, directory_names, file_names in os.walk(root, followlinks=False):
        ...
        for name in file_names:
            path = current / name
            if _path_is_link_like(path) or suffix is None or path.suffix.lower() == suffix:
                candidates.append(path)
    return candidates
...
def _unexpected_plugin_entrypoints(destination: pathlib.Path) -> list[str]:
    """列出 Claude 会自动发现、但本插件没有声明的运行入口。"""
    commands_root = destination / "commands"
    skills_root = destination / "skills"
    agents_root = destination / "agents"
    candidates = [
        *_component_entrypoints(commands_root, ".md"),
        *_skill_entrypoints(skills_root),
        *_component_entrypoints(agents_root, ".md"),
    ]
    ...
    for relative in CLAUDE_EMPTY_COMPONENT_DIRECTORIES:
        ...
        candidates.extend(_component_entrypoints(root, None))

    allowed = CLAUDE_COMMAND_ENTRYPOINTS | PUBLIC_SKILL_ENTRYPOINTS
    return sorted(
        {
            path.relative_to(destination).as_posix()
            for path in candidates
            if path.relative_to(destination).as_posix() not in allowed
        }
    )
...
def _copy(source: pathlib.Path, destination: pathlib.Path) -> None:
    ...
    try:
        ...
        if os.name != "nt" and hasattr(os, "fchmod"):
            desired_mode = stat.S_IMODE(source_details.st_mode)
            os.fchmod(destination_descriptor, desired_mode)
...
def _validate_adapter(destination: pathlib.Path) -> None:
    ...
    required = (
        ...
        destination / "commands" / "doctor.md",
        destination / "commands" / "check-diff.md",
        destination / "commands" / "help.md",
    ...
    if linked:
        raise RuntimeError(
            "Claude 适配包包含未声明公开入口（链接型必需资源）：" + ", ".join(linked)
        )
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise FileNotFoundError("Claude 适配包缺少资源：" + ", ".join(missing))
    plugin_manifest = _validate_manifest(destination / ".claude-plugin" / "plugin.json")
    ...
    if unexpected:
        raise RuntimeError("Claude 适配包包含未声明公开入口：" + ", ".join(unexpected))
    _safe_tree_snapshot(destination)
...
def _source_package_snapshot() -> dict[str, dict[str, object]]:
    ...
    for source, prefix in (
        (root / ".claude-plugin", ".claude-plugin"),
        (root / "hooks", "hooks"),
...
def _validate_managed_destination(destination: pathlib.Path) -> dict[str, object]:
    ...
    if not manifest_path.is_file():
        raise RuntimeError(f"现有安装目标缺少 Claude manifest，拒绝覆盖：{destination}")
    manifest = _validate_manifest(manifest_path)
    if manifest.get("name") != PLUGIN_NAME:
        raise RuntimeError(f"现有安装目标不是受管 Claude 插件：{destination}")
    ...
    if _path_exists_without_following(marker):
        ...
        if actual_digest != current_digest and actual_digest not in historical_digests:
            raise RuntimeError(
                "现有 Claude 安装缺少 ownership marker，且完整内容摘要不符合当前或已知历史发布契约："
                f"{destination}"
            )
        final_snapshot = _safe_tree_snapshot(destination)
    if final_snapshot != initial_snapshot:
        raise RuntimeError(f"现有 Claude 安装目录树在校验期间发生变化：{destination}")
...
def _build_adapter(root: pathlib.Path, staging: pathlib.Path) -> None:
    ...
    if set(initial_tree) != {"."}:
        raise RuntimeError(f"Claude staging 构建前不是空目录：{staging}")
    ...
    manifest_directory = staging / ".claude-plugin"
    manifest_directory.mkdir()
    manifest_directory_identity = _path_identity(manifest_directory)
    _copy(root / ".claude-plugin" / "plugin.json", manifest_directory / "plugin.json")
    _copy(root / ".claude-plugin" / "marketplace.json", manifest_directory / "marketplace.json")
    _assert_identity(manifest_directory, manifest_directory_identity, "Claude manifest 目录")
    ...
    if set(unsigned_tree) != set(source_tree) or _tree_content_digest(
        ...
        raise RuntimeError(f"Claude staging 内容不符合当前发布源契约：{staging}")
